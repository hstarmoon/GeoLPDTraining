...
def label_transfer(img, label, offset_start, offset):
    ...
    height = round((br[1] - tl[1]) * (offset + 0.2))
...
def main(argv):
    ...
    parser.add_argument('--data_dir', type=str, default=r'D:\RD\Project\GeoLPDTraining-master\TD\tools\123',
                        required=False,
                        help='Path to input data\n')
    parser.add_argument('--output_dir', type=str, default=r'D:\RD\Project\GeoLPDTraining-master\TD\tools\456',
                        required=False,
            ...
            for txtfiles in glob.glob(os.path.join(root, "*.txt")):
                ...
                if (labels):
                    ...
                    for i, float_label in enumerate(float_labels):
                        crop_img, label_crop = label_transfer(img, float_label, args.offset - 10, args.offset)
                        save_jpgandtxt(crop_img, label_crop, os.path.join(savepath,
                                                                          os.path.relpath(os.path.dirname(txtfiles),
                                                                                          args.data_dir)),
                                       '_' + str(i) + '_10_' + os.path.basename(txtfiles))

                        crop_img, label_crop = label_transfer(img, float_label, args.offset, args.offset + 10)
                        save_jpgandtxt(crop_img, label_crop, os.path.join(savepath,
                                                                          os.path.relpath(os.path.dirname(txtfiles),
                                                                                          args.data_dir)),
                                       '_' + str(i) + '_' + os.path.basename(txtfiles))
